class BotJob < ApplicationJob
  ...
  def perform(*args)
    ...
    bot.message(with_text: '!help') do |event|
      event.respond "@#{event.user.display_name}, the command list is:"
    ...
    bot.message(with_text: '!register') do |event|
      ...
      if player
        ...
        event.respond "@#{event.user.display_name}, you are now in the current queue for the next hour."
        ...
        if current_registrations.count < 16
          ...
          event.respond "@#{event.user.display_name}, we need #{players_required} more players to designate captains."
        elsif current_registrations.count.eql?(16)
          event.respond "@#{event.user.display_name}, you are the 16th player! I proceed to the captains designation..."
          ...
          event.respond "@#{event.user.display_name}, the captains are @#{scrim.captain_a.username} and @#{scrim.captain_b.username}."
          ...
          event.respond "@#{event.user.display_name}, you are on the waiting list."
        end
      else
        event.respond "@#{event.user.display_name}, you need to log in with Discord on https://gwrank.com to register yourself."
      end
    end
    ...
    bot.message(with_text: '!unregister') do |event|
      ...
      if player
        if player.has_current_registration?
          ...
          event.respond "@#{event.user.display_name}, you are not anymore in the current queue."
        else
          event.respond "@#{event.user.display_name}, you were not in the current queue."
        end
      else
        event.respond "@#{event.user.display_name}, you need to log in with Discord on https://gwrank.com to unregister yourself."
      end
    end
    ...
    bot.message(with_text: '!captains') do |event|
      ...
      if current_registrations.count >= 16
        ...
        event.respond "@#{event.user.display_name}, the current captains are @#{scrim.captain_a.username} and @#{scrim.captain_b.username}."
      else
        players_required = 16 - current_registrations.count
        event.respond "@#{event.user.display_name}, we need #{players_required} more players to designate captains."
      end
